...
from samantha.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import samantha.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `samantha? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

Samantha will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^samantha\?"), decide, friendly="decide"
)
