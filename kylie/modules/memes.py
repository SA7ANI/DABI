...
from kylie.modules.helper_funcs.extraction import extract_user
...
from kylie.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import kylie.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `kylie? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

KYLIE will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^kylie\?"), decide, friendly="decide"
)
