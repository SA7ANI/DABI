...
from hinata import dispatcher, LOGGER
from hinata.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import hinata.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `hinata? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

Hinata will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^hinata\?"), decide, friendly="decide"
)
