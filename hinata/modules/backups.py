...
from hinata import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
...
def export_data(update, context):
    ...
    f = open("Hinata{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Hinata{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `hinata-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Hinata{}.backup".format(chat_id))  # Cleaning file
