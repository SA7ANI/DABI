...
from spike.modules.helper_funcs.string_handling import markdown_parser
from spike.modules.disable import DisableAbleCommandHandler

import spike.modules.sql.feds_sql as sql

from spike.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "spike_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="spike_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "spike_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="spike_fbanned_users.csv",
