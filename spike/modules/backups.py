...
import spike.modules.sql.notes_sql as sql
from spike import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from spike.__main__ import DATA_IMPORT
from spike.modules.helper_funcs.chat_status import user_admin
from spike.modules.helper_funcs.alternate import typing_action

# from spike.modules.rules import get_rules
import spike.modules.sql.rules_sql as rulessql

# from spike.modules.sql import warns_sql as warnssql
import spike.modules.sql.blacklist_sql as blacklistsql
from spike.modules.sql import disable_sql as disabledsql

# from spike.modules.sql import cust_filters_sql as filtersql
# import spike.modules.sql.welcome_sql as welcsql
import spike.modules.sql.locks_sql as locksql
from spike.modules.connection import connected
...
def export_data(update, context):
    ...
    f = open("Spike{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Spike{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Spike-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Spike{}.backup".format(chat_id))  # Cleaning file
