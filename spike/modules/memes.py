...
from spike.modules.helper_funcs.extraction import extract_user
...
from spike.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import spike.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `spike? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

Spike will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^spike\?"), decide, friendly="decide"
)
