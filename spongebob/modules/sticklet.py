...
from spongebob import LOGGER, client
import random
...
@client.on(events.NewMessage(pattern="^/bang (.*)"))
async def bang(e):
    if e.is_group:
    ...
    if not sticktext:
    	...
    	sticktext = get.text
    if not sticktext:
    	await e.reply("`I need text to make sticker !`")
    	return
    ...
    fontsize = 230
    font = ImageFont.truetype("/root/spongebob/spongebob/modules/helper_funcs/DejaVuSansMono.ttf", size=fontsize)
    while draw.multiline_textsize(sticktext, font=font) > (512, 512):
        fontsize -= 3
        font = ImageFont.truetype(".spongebob/modules/helper_funcs/DejaVuSansMono.ttf", size=fontsize)
    ...
    image_stream.seek(0)
    await e.client.send_file(e.chat_id, image_stream)